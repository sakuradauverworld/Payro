...
def test_remove_group(data_dir):
    mgr = RecipientManager(data_dir)
    group = mgr.add_group("テスト", "default")
    mgr.add_recipient(group.id, Recipient(name="山田", email="yamada@example.com"))
...
def test_remove_recipient(data_dir):
    ...
    group = mgr.add_group("テスト", "default")
    mgr.add_recipient(group.id, Recipient(name="山田", email="yamada@example.com"))
    mgr.add_recipient(group.id, Recipient(name="佐藤", email="sato@example.com"))
...
def test_update_recipient(data_dir):
    ...
    group = mgr.add_group("テスト", "default")
    mgr.add_recipient(group.id, Recipient(name="山田", email="yamada@example.com"))
    mgr.update_recipient(group.id, 0, Recipient(name="山田太郎", email="new@example.com"))
...
def test_recipient_excluded_default_false():
    r = Recipient(name="山田", email="yamada@example.com")
